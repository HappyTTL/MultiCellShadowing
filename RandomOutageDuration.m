...
%         BaseStationPosition = BaseStationPoissonProc-500;
     ...
     BaseStationPosition = [BaseStationPosition_X-r/2 BaseStationPosition_Y-r/2];
        ...
        for i = 1 : N(1)
            FieldID = ceil(9 * rand);
            for j = 1 : r
                Shadowing_DB(i,j) = ShadowField(FieldID, round(BaseStationPosition(i,1))+1500, round(BaseStationPosition(i,2))+1500)*Sigma;
    ...
    filename = ['Max_SINR_Outage' num2str(Lambda) '.mat'];
    save(filename, 'Max_SINR');
    filename = ['Max_SIR_Outage' num2str(Lambda) '.mat'];
