...
SINR_Threshold = -10; %%%%%SINR threshold is 10db
alpha = 4;
N0 = -104;
...
for m = 1 : r
    for n = 1 : r
        ...
        for i = 1 : Lambda
            RxPower_WithShadow(i) = 10^(P0/10) * 0.001 * PL(i) * ShadowFading(i);
%             * Rayleigh(i);
...
figure(2);
imagesc(SINR_Marker_NoShadow);
axis([0 r 0 r]);
colormap(gray);
title('Outage Area without Shadow Fading');
figure(3);
